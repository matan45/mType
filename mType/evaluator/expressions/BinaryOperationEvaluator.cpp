...
namespace evaluator
{
    namespace expressions
    {
        ...
        Value BinaryOperationEvaluator::evaluateComparison(const Value& left, const Value& right, TokenType op)
        {
            ...
            {
                bool leftNull = std::holds_alternative<std::monostate>(left) || std::holds_alternative<
                    std::nullptr_t>(left);
                bool rightNull = std::holds_alternative<std::monostate>(right) || std::holds_alternative<
                    std::nullptr_t>(right);
            ...
            if (std::holds_alternative<InternedString>(left) && std::holds_alternative<
                InternedString>(right))
            {
                std::string leftStr = std::get<InternedString>(left).getString();
                std::string rightStr = std::get<InternedString>(right).getString();
            ...
            if ((std::holds_alternative<std::string>(left) && std::holds_alternative<InternedString>(right)) ||
                (std::holds_alternative<InternedString>(left) && std::holds_alternative<std::string>(right)))
            {
                std::string leftStr = std::holds_alternative<std::string>(left)
                                          ? std::get<std::string>(left)
                                          : std::get<InternedString>(left).getString();
                std::string rightStr = std::holds_alternative<std::string>(right)
                                           ? std::get<std::string>(right)
                                           : std::get<InternedString>(right).getString();
                ...
                std::holds_alternative<std::shared_ptr<NativeArray>>(right))
            {
                ...
                default:
                    throw TypeException("Cannot use relational operators (<, >, <=, >=) with objects",
                                        SourceLocation{});
                }
            }
