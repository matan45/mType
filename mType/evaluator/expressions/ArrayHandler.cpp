    ...
    namespace expressions
    {
            ...
            for (const auto& element : elements)
            {
                ...
                else
                {
                    ...
                    if (currentType != expectedType)
                    {
                        ...
                        {
                            throw TypeException("Cannot mix null values with primitive types in array literals",
                                                node->getLocation());
                        }
                        else
                        {
            ...
            IndexAccessNode* node, std::vector<size_t>& indices)
        {
            ...
            else if (std::holds_alternative<std::shared_ptr<mType::value::arrays::FlatMultiObjectArray>>(baseArray))
            {
                auto flatMultiObjectArray = std::get<std::shared_ptr<mType::value::arrays::FlatMultiObjectArray>>(
                    baseArray);
                isMultiDimensional = flatMultiObjectArray->getRank() > 1;
            }
                                                                 ...
                                                                 const SourceLocation& location)
        {
            ...
            if (std::holds_alternative<std::shared_ptr<SparseMultiArray>>(baseArray))
            {
                ...
                {
                    throw TypeException("Sparse multi-dimensional array access failed: " + std::string(e.what()),
                                        location);
                }
            }

            // Handle FlatMultiObjectArray direct access
            if (std::holds_alternative<std::shared_ptr<mType::value::arrays::FlatMultiObjectArray>>(baseArray))
            {
                ...
                {
                    throw TypeException(
                        "FlatMultiObjectArray multi-dimensional access failed: " + std::string(e.what()), location);
                }
            }
