...
namespace vm::compiler
{
    ...
    void BytecodeCompiler::registerClassesForBytecode(ast::ASTNode* node)
    {
        ...
        // Check if this node is an InterfaceNode FIRST (before ClassNode)
        if (auto interfaceNode = dynamic_cast<ast::nodes::classes::InterfaceNode*>(node))
        {
            registerInterfaceForBytecode(interfaceNode);
            return; // No need to traverse children of InterfaceNode
        }
        ...
        if (auto classNode = dynamic_cast<ast::ClassNode*>(node))
        {
            ...
            // Validate interface implementations AFTER registration
            validateInterfaceImplementations(classDef, classNode->getLocation());
    ...
    void BytecodeCompiler::registerInterfaceForBytecode(ast::nodes::classes::InterfaceNode* interfaceNode)
    {
    ...
    void BytecodeCompiler::validateInterfaceImplementations(
    ...
    std::pair<std::string, std::vector<std::string>> BytecodeCompiler::parseGenericInterfaceName(
        const std::string& interfaceName)
    {
        ...
        if (closeBracket == std::string::npos) {
            throw errors::TypeException("Malformed generic interface name: " + interfaceName);
        }
    ...
    std::string BytecodeCompiler::resolveGenericType(
        const std::string& typeName,
        const std::unordered_map<std::string, std::string>& substitutions)
    {
        // Check if this type name is a generic parameter that should be substituted
