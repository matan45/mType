...
namespace vm::jit
{
                                      ...
                                      JitCodeCache& codeCache)
    {
        ...
        bool usesBoxedTypes = false;
        {
            ...
            // Check if CALL returns non-primitive
            if (!usesBoxedTypes)
            {
                for (size_t ip = loopStartOffset; ip <= loopEndOffset; ++ip)
                {
            ...
            // Check local slot types from captured state
            if (!usesBoxedTypes)
            {
                for (const auto& s : localSlotInfos)
                {
                    if (isBoxedSlotType(s.type))
                    {
                        usesBoxedTypes = true;
                        break;
                    }
        ...
        // Program pointer for constant pool access
        Gp progPtr = cc.new_gp64("progPtr");
        if (usesBoxedTypes)
        {
            cc.mov(progPtr, reinterpret_cast<uint64_t>(&program));
        ...
        for (size_t ip = loopStartOffset; ip <= loopEndOffset; ++ip)
        {
                ...
                instr.opcode == OpCode::JUMP_IF_TRUE || instr.opcode == OpCode::JUMP_BACK)
            {
                ...
                {
                    size_t target = instr.operands[0];
                    // Only create labels for targets within the loop body
        ...
        // Label for loop exit write-back
        Label exitLabel = cc.new_label();
        size_t resumeOffset = loopEndOffset + 1;

        // Compile-time operand stack depth
        int stackDepth = 0;

        // Helper lambdas — same as in compile()
        auto emitBox = [&](Gp destAddr, int stackOff, SlotType type) {
            if (type == SlotType::FLOAT)
            {
                Vec val = cc.new_xmm();
        ...
        // Helper: emit locals write-back to ctx->osrOutputLocals
        auto emitLocalsWriteBack = [&]() {
            Gp outPtr = cc.new_gp64();
            cc.mov(outPtr, Mem(ctxPtr, offsetof(JitContext, osrOutputLocals)));
            ...
            for (size_t i = 0; i < localCount; ++i)
            {
                ...
                SlotType lt = localTypes.count(i) ? localTypes[i] : SlotType::INT;

                if (usesBoxedTypes)
                {
                    // In boxed mode, locals are Value-sized — copy directly
                    Gp srcAddr = cc.new_gp64();
                    cc.lea(srcAddr, Mem(localsBase, static_cast<int32_t>(i * localStride)));
                    InvokeNode* inv;
                    cc.invoke(Out(inv), reinterpret_cast<uint64_t>(jit_value_copy),
                              FuncSignature::build<void, value::Value*, const value::Value*>());
                    inv->set_arg(0, destAddr);
                ...
                {
                    // Non-boxed mode: box the 8-byte local into a Value
                    if (lt == SlotType::FLOAT)
                    {
                        Vec val = cc.new_xmm();
                        cc.movss(val, Mem(localsBase, static_cast<int32_t>(i * 8)));
                        InvokeNode* inv;
                        cc.invoke(Out(inv), reinterpret_cast<uint64_t>(jit_box_float),
                                  FuncSignature::build<void, value::Value*, float>());
                        inv->set_arg(0, destAddr);
                    ...
                    {
                        Gp val = cc.new_gp64();
                        cc.mov(val, Mem(localsBase, static_cast<int32_t>(i * 8)));
                        InvokeNode* inv;
        ...
        // ===== Main codegen loop for the loop body =====
        for (size_t ip = loopStartOffset; ip <= loopEndOffset && !compileFailed; ++ip)
        {
            auto labelIt = labels.find(ip);
            if (labelIt != labels.end())
                cc.bind(labelIt->second);
            ...
            switch (instr.opcode)
            {
                ...
                {
                    size_t target = instr.operands[0];
                    if (target > loopEndOffset)
                    {
                        // Break: jump outside loop → write back locals and exit
                        emitLocalsWriteBack();
                        // Set exit offset to the target (after LOOP_END)
                        cc.mov(Mem(ctxPtr, offsetof(JitContext, osrExitOffset)),
                               static_cast<int64_t>(target));
                        cc.mov(Mem(ctxPtr, offsetof(JitContext, osrExited)), 1);
                        emitCleanup();
                        cc.ret();
                    }
                    else
                ...
                case OpCode::JUMP_IF_FALSE:
                {
                    ...
                    if (target > loopEndOffset)
                    {
                        // Loop exit condition: if false, exit the loop
                        Label continueLoop = cc.new_label();
                        cc.jnz(continueLoop);

                        // Exit path: write back locals
                        emitLocalsWriteBack();
                        cc.mov(Mem(ctxPtr, offsetof(JitContext, osrExitOffset)),
                               static_cast<int64_t>(target));
                        cc.mov(Mem(ctxPtr, offsetof(JitContext, osrExited)), 1);
                ...
                case OpCode::RETURN_VALUE:
                {
                    // If a return is inside the loop (shouldn't happen normally),
                    // write back and exit
                    ...
                    emitLocalsWriteBack();
                    cc.mov(Mem(ctxPtr, offsetof(JitContext, osrExitOffset)),
                           static_cast<int64_t>(resumeOffset));
                    cc.mov(Mem(ctxPtr, offsetof(JitContext, osrExited)), 1);
                    emitCleanup();
                    cc.ret();
                    break;
                ...
                case OpCode::CALL:
                {
                    ...
                    {
                        if (funcName == "print" || funcName == "println") returnType = "void";
                        else { compileFailed = true; break; }
                    }
                    bool isPrimReturn = (returnType == "int" || returnType == "float" ||
                                         returnType == "bool" || returnType == "void");
                    if (!isPrimReturn && !usesBoxedTypes) { compileFailed = true; break; }
                ...
                // Phase 4 opcodes (string, object, array, etc.) — same as compile()
                // but not duplicated here for brevity. If the loop uses these opcodes,
                // canCompileLoopOSR already verified they're supported.
                case OpCode::PUSH_STRING:
                {
                    uint32_t constIndex = static_cast<uint32_t>(instr.operands[0]);
                    Gp dest = cc.new_gp64();
                ...
                case OpCode::CAST:
                case OpCode::NEW_OBJECT:
                    // These opcodes are supported but complex to duplicate.
                    // For V1, bail out for loops that use them in OSR path.
                    // The canCompileLoopOSR check already passed, but we defer
                    // full Phase 4 OSR support to a later iteration.
                    compileFailed = true;
                    break;
        ...
        // Fallthrough cleanup (shouldn't normally reach here — loop should exit via JUMP_IF_FALSE)
        emitCleanup();
        cc.mov(Mem(ctxPtr, offsetof(JitContext, hasReturnValue)), 0);
        cc.end_func();
