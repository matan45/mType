...
std::vector<CompletionItem> PathCompletionHandler::getPathCompletions(
    ...
    std::cerr << "[PathCompletionHandler] getPathCompletions called for uri: " << uri
    ...
    if (!doc) {
        std::cerr << "[PathCompletionHandler] Document not found" << std::endl;
        return items;
    }

    // Check if we're inside an import string
    bool insideImport = isInsideImportString(doc->content, position);
    std::cerr << "[PathCompletionHandler] Inside import string: " << (insideImport ? "YES" : "NO") << std::endl;
    ...
    std::cerr << "[PathCompletionHandler] Extracted path: '" << currentPath << "'" << std::endl;

    // Resolve the path relative to the current file
    std::string resolvedPath = resolveRelativePath(uri, currentPath);
    std::cerr << "[PathCompletionHandler] Resolved to directory: '" << resolvedPath << "'" << std::endl;

    // List directory contents
    auto completions = listDirectoryContents(resolvedPath);
    std::cerr << "[PathCompletionHandler] Found " << completions.size() << " completion items" << std::endl;
